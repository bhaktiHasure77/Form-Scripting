...
// Map numerical values to readable labels for better code clarity
   ...
   // Runs when the record form is opened
    static async onFormLoad(params: Xrm.Events.EventContext): Promise<void> {
    ...
    static onFormSave(params: Xrm.Events.SaveEventContext): void {
      const formContext = params.getFormContext();
      const teamLeaderAttr = formContext.getAttribute(
        ProjectForm.Fields.TeamLeader,
      );

      if (teamLeaderAttr) {
        const leader = teamLeaderAttr.getValue() as Xrm.LookupValue[];
      // Ensure a Team Leader is present; if not, block the save and show an error
        if (!leader || leader.length === 0) {
          Xrm.Navigation.openErrorDialog({
            message:
              "A Team Leader must be assigned before starting the project and prevent record form saving.",
          });
    ...
    static async onStatusChange(
      params: Xrm.Events.EventContext,
    ): Promise<void> {
      const formContext = params.getFormContext();
      ProjectForm.handleProjectCompletionPercentage(formContext);
    ...
    static async onTypeOrDeadlineChange(
      params: Xrm.Events.EventContext,
    ): Promise<void> {
      const formContext = params.getFormContext();
      ProjectForm.validateDeadline(formContext);
    ...
    static onCompletionPercentageChange(params: Xrm.Events.EventContext): void {
      const formContext = params.getFormContext();
      const statusAttr = formContext.getAttribute(ProjectForm.Fields.Status);
      const pctAttr = formContext.getAttribute(
        ProjectForm.Fields.CompletionPercentage,
      );
      const pctCtrl = formContext.getControl(
        ProjectForm.Fields.CompletionPercentage,
      ) as Xrm.Controls.NumberControl;

      // Ensure percentage is between 1 and 99 when the project is active
      if (statusAttr && pctAttr && pctCtrl) {
        const status = statusAttr.getValue() as number;
        const pct = pctAttr.getValue() as number;
        ...
        if (status === ProjectForm.Enums.Status.InProgress) {
          if (pct !== null && (pct < 1 || pct > 99)) {
            pctCtrl.setNotification(
              "Percentage must be between 1 and 99 when In Progress.",
              "pct_err",
            );
          } else {
    ...
    // REQUIREMENTS
    // Deadline Management
    private static validateDeadline(formContext: Xrm.FormContext): void {
      const typeAttr = formContext.getAttribute(ProjectForm.Fields.Type);
      const statusAttr = formContext.getAttribute(ProjectForm.Fields.Status);
      const deadlineAttr = formContext.getAttribute(
        ProjectForm.Fields.Deadline,
      );
      const deadlineCtrl = formContext.getControl(
        ProjectForm.Fields.Deadline,
      ) as Xrm.Controls.DateControl;

      if (typeAttr && statusAttr && deadlineAttr && deadlineCtrl) {
        const type = typeAttr.getValue() as number;
        const status = statusAttr.getValue() as number;
        const deadline = deadlineAttr.getValue() as Date;

        if (
          type === ProjectForm.Enums.Type.Client &&
          ...
          if (diffDays > 30) {
            ...
            deadlineCtrl.setNotification(
              "Deadline is over 30 days.",
              "deadline_err",
            );
          } else {
    ...
    private static handleProjectCompletionPercentage(
    ...
    private static async handleAutomaticResourceAllocation(
      formContext: Xrm.FormContext,
    ): Promise<void> {
      const typeAttr = formContext.getAttribute(ProjectForm.Fields.Type);
      const priorityAttr = formContext.getAttribute(
        ProjectForm.Fields.Priority,
      );

      if (typeAttr && priorityAttr) {
        const type = typeAttr.getValue() as number;
        const priority = priorityAttr.getValue() as number;
        let teamName = "";
        ...
        if (teamName) {
          try {
            const results = await Xrm.WebApi.retrieveMultipleRecords(
              "team",
              `?$select=teamid&$filter=name eq '${teamName}'`,
            );
            if (results.entities.length > 0) {
              const teamId = results.entities[0].teamid;
              const members = await Xrm.WebApi.retrieveMultipleRecords(
                "teammembership",
    ...
    private static async handleTeamLeaderAssignment(
      formContext: Xrm.FormContext,
    ): Promise<void> {
      const typeAttr = formContext.getAttribute(ProjectForm.Fields.Type);
      const statusAttr = formContext.getAttribute(ProjectForm.Fields.Status);
      const clientAttr = formContext.getAttribute(
        ProjectForm.Fields.ClientName,
      );

      if (typeAttr && statusAttr && clientAttr) {
        const type = typeAttr.getValue() as number;
        const status = statusAttr.getValue() as number;
        const client = clientAttr.getValue() as Xrm.LookupValue[];
          ...
          if (id) {
            try {
              const res = await Xrm.WebApi.retrieveRecord(
                "account",
                id,
                "?$select=_hd_accountmanager_value",
              );
              const mId = res["_hd_accountmanager_value"];
              const mName =
                res[
                  "_hd_accountmanager_value@OData.Community.Display.V1.FormattedValue"
                ];
              if (mId) {
                formContext
